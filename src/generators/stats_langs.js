...
async function fetchTopLanguages() {
  ...
  `;
	...
	const totalBytes = Object.values(languages).reduce((sum, lang) => sum + lang.size, 0);
	...
	const filteredTotal = filteredEntries.reduce((sum, [_, lang]) => sum + lang.size, 0);
	...
	const filteredLanguages = filteredEntries
		.map(([name, lang]) => ({
			...
			percentFixed:
				filteredTotal > 0 ? ((lang.size / filteredTotal) * 100).toFixed(2) : "0.00",
			color: lang.color,
		}))
